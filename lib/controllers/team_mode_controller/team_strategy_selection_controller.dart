...
class TeamStrategySelectionController extends GetxController {
  ...
  /// API response
  final Rxn<TeamStrategyResponse> teamStrategyResponse =
      Rxn<TeamStrategyResponse>();
  ...
  final TeamGameTimerController _timerController =
      Get.find<TeamGameTimerController>(); // <--- NEW FIND
  // -----------------------------------------------------------
  ...
  Future<void> fetchAndRevealStrategy() async {
    ...
    if (isCardRevealed.value || isLoading.value) {
      print(
        'Exiting: isRevealed=${isCardRevealed.value}, isLoading=${isLoading.value}',
      );
      return;
    }
    try {
      ...
      if (teamId == null) {
        Get.snackbar(
          'Error',
          'Team ID missing. Please ensure a team is created and selected.',
        );
      ...
      final response = await _strategyRepository.getTeamStrategy(
        teamId: teamId,
        role: role,
      );
      ...
      if (response.title != null) {
        ...
        if (response.remainingTime != null) {
          ...
          print(
            'Timer initialized with ${response.remainingTime!.minutes}:${response.remainingTime!.seconds}',
          );
        }
      } else {
        ...
        Get.snackbar(
          'Error',
          response.message ?? 'No strategy found for your team.',
        );
