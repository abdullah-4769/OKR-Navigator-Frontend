...
class TeamSuggestionInitiativesController extends GetxController {
  ...
  // Add this method to your TeamSuggestionInitiativesController class
  KeyResult getRandomKeyResult() {
    if (keyResults.isEmpty) {
      return KeyResult(
        title: 'No Key Results',
        description: 'Please select key results first',
      );
    }

    final random = Random();
    final randomIndex = random.nextInt(keyResults.length);
    return keyResults[randomIndex];
  }
  ...
  Future<void> submitInitiatives() async {
    ...
    try {
      ...
      final strategyTitle =
          Get.find<TeamStrategySelectionController>().selectedStrategy.value;
      final objectiveTitle =
          Get.find<TeamObjectiveController>().selectedObjective.value!.title!;
      final language =
      ...
      Get.find<TeamContextualChallengeController>().finalInitiatives.assignAll(
        initiatives,
      );
      ...
      if (isChallengeMode.value) {
        ...
        isSubmitting.value =
            false; // <-- FIX: Explicit reset for challenge flow.
      ...
      if (!isChallengeMode.value) {
        // Only reset here if not challenge mode (where it was already reset)
