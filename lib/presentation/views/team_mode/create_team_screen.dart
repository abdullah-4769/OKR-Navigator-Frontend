...
class CreateTeamScreen extends StatelessWidget {
 ...
 Widget build(BuildContext context) {
    // 💡 NEW: Retrieve the argument to determine if we are in edit mode
  ...
  // 💡 NEW: Load data if in edit mode (once the controller is put)
  if (isEditing) {
      // Check if we have a team ID and haven't loaded yet (check if controller text fields are empty as a proxy)
      if (controller.teamNameController.text.isEmpty && controller.createdTeamId.value != null) {
          controller.loadTeamDetailsForEdit(controller.createdTeamId.value!);
      }
  }
...
SizedBox(
              ...
              Obx(() => CustomButton(
               ...
               onPressed: controller.isCreatingTeam.value 
                        ? () {} 
                        : () => controller.continueCreateTeam(isEditing: isEditing), // Pass flag
              )),
             ],
            ),
           ),
