...
class GameResultController extends GetxController {
  ...
  Future<void> fetchGameResult(String challengeId, String userId) async {
    try {
      ...
      final url = '${ApiConstants.challengeModeScore}?challenge=$challengeId&userId=$userId';
      ...
      if (response.statusCode == 200) {
        ...
        pointsBreakdown.assignAll(
          List<Map<String, dynamic>>.from(data['pointsBreakdown'] ?? []),
        );
...
//         Uri.parse("http://localhost:3000/challenge-mode-score?challenge=$challengeId&userId=$userId"),
//       );
//
//       if (response.statusCode == 200) {
//         final data = jsonDecode(response.body);
